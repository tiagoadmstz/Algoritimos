...
public class ControleUsuario {
     ...
     */
    public static void tentativasAcesso(JFrame frame, Usuario usuario) {
        ...
        if (USUARIOS.get(usuario.getUsuario()) == 10) {
            ...
            Usuario user = (Usuario) emh.getObject("from Usuario as user where user.usuario = :paramUser", "paramUser", usuario.getUsuario(), EntityManagerHelper.DERBYDB_PU);
            if (user != null) {
                user.setBloqueado(true);
                emh.getOperation(EntityManagerHelper.UPDATE, user, EntityManagerHelper.DERBYDB_PU);
            }
            USUARIOS.replace(usuario.getUsuario(), USUARIOS.get(usuario.getUsuario()) + 1);
        }
